...
- **`users/{uid}`**: leitura liberada pra qualquer logado (necessário pra `useUserProfiles` resolver nome de qualquer membro), escrita só pelo próprio uid.
...
## 5. Checklist de segurança para toda mudança
...
4. Mudança que toca o fluxo de SSO (seção 4) é revisada nos dois repos (CashZ e planning-trip), nunca só num lado.

...
Planejamento pessoal de moeda estrangeira — "quanto já comprei" vs. "quanto as despesas em moeda estrangeira precisam" — **não** um caixa com consumo/reserva real. Decisões de produto que moldaram o desenho (ver [DATABASE.md](./DATABASE.md) 2.5 pro modelo de dado):

- **A própria moeda da despesa é o sinal — sem campo/checkbox separado**: `TripWalletSection` (dentro de `WalletPage.tsx`) filtra `currency !== 'BRL'` sobre as despesas de `useTrip(tripId)`. Achado do usuário testando: uma marcação manual (`paidFromWallet`, existiu numa 1ª versão) era redundante — registrar em EUR/GBP/etc. já significa "preciso ter esse dinheiro físico no destino", nunca "posso pagar em real". Consequência: `AddExpenseDialog.tsx` não pergunta mais taxa/spread pra despesa não-BRL (removido o editor de "Cotação de compra"/presets Wise/Cartão) — `amountBRL` sempre usa cotação de mercado, sem escolha manual, porque não existe mais o conceito de "conversão na hora com taxa de cartão" separado da carteira.
...
- **Por viagem e por participante**, não persiste entre viagens, e só se aplica a viagens com moeda de referência estrangeira (`Trip.baseCurrency !== 'BRL'`) — viagem doméstica não tem esse conceito, mesmo que uma despesa avulsa tenha sido lançada noutra moeda (`WalletPage.tsx` filtra a lista de viagens).
- **Multi-moeda numa mesma viagem já funciona sem tratamento especial**: `summarizeWalletDemand` agrupa por `currency` — uma viagem passando por Londres (GBP) e França (EUR) mostra 2 cards separados, cada um com seu próprio comprado/necessário/falta.
- **Nunca bloqueia**: uma despesa não-BRL conta como demanda mesmo sem saldo comprado suficiente — o que falta vira um número visível (`shortfall` em `summarizeWalletDemand`, `src/lib/currencyWallet.ts`), nunca um erro. Por isso não existe consumo/reversão de lote — é aritmética pura recalculada a cada leitura, bem mais simples que a 1ª versão cogitada (FIFO com bloqueio, descartada em conversa com o usuário).
- **Necessário é a cota do pool, não o valor cheio da despesa nem quem pagou**: achado real — uma despesa de €750 dividida entre 3 pessoas, registrada de uma vez (`participants` com os 3 uids), deve contar só a cota de quem está no pool (€250 se 1 pessoa do pool participa, €500 se 2), nunca os €750 inteiros nem só pra quem é `paidBy`. `TripWalletSection` busca as despesas da viagem com `useTrip(tripId)` (não filtra por `participants`/`paidBy` na query — ver achado de segurança abaixo) e filtra client-side por `currency !== 'BRL' && participants.some(p => poolUids.includes(p))`; `summarizeWalletDemand(lots, expenses, poolUids)` calcula `amountOriginal / participants.length` × quantos uids do pool estão em `participants` daquela despesa. Cada pessoa precisa ter em mãos a própria cota, independente de quem historicamente pagou a conta.
- **Privada por padrão, compartilhável por decisão mútua** (`wallet_shares`, ver [DATABASE.md](./DATABASE.md) 2.6): `firestore.rules` libera leitura pro próprio `ownerUid`, pro owner/editor da viagem quando o dono é um fantasma (`ghost_*`), **ou** quando existe um `WalletShareDeclaration` nos dois sentidos entre os dois uids pra aquele `tripId` (`hasMutualWalletShare()` na regra — duas chamadas `exists()` por doc ID determinístico, nunca query dentro da regra). Caso de uso real: casal viajando junto com dinheiro unificado — cada um declara unilateralmente ("quero juntar com X"), o pool só ativa quando os dois lados existem. Sem a marcação manual, quem pode "decidir que uma despesa é carteira" deixou de ser uma questão — é só a moeda escolhida na hora de criar a despesa, mesma autoridade de qualquer outro campo (owner/editor).
...
- **Tela global na URL (`/wallet`), mas leitura sempre por viagem**: `WalletPage.tsx` lista as viagens não-BRL do usuário (`useUserTrips()`) e renderiza um `TripWalletSection` por viagem — cada instância chama `useCurrencyLots(trip.id, poolUids)` e `useTrip(trip.id)` (reaproveitado, já buscava `expenses` por `tripId`) **próprios**, nunca uma leitura única achatada. Achado de segurança real (2 rodadas): uma query de lista no Firestore só passa se a regra puder ser provada pro conjunto **potencial** de resultados da query, não documento por documento com dado real — `where('participants','array-contains-any',[...])` em `expenses` falhava porque a regra de `expenses` só depende de `tripId`, não `participants`; corrigido reaproveitando `useTrip(tripId)`. Já `currency_lots` tem uma regra que depende de **dois** campos (`tripId` e `ownerUid`), então nem `where('tripId','==',tripId)` sozinho nem `where('ownerUid','in',poolUids)` sozinho bastam — precisa dos dois juntos (`useCurrencyLots(tripId, poolUids)`). Quem é dono do lote/participa do pool na despesa continua filtrado client-side depois da leitura (`poolUids.includes(...)`) só pra granularidade fina (ex. por moeda), nunca como única defesa. Ver [SECURITY.md](./SECURITY.md).
- **Acesso bloqueado de verdade pra quem não tem viagem em moeda estrangeira**, não só escondido: `Layout.tsx` só mostra "Carteira" no dropdown do avatar quando `useUserTrips()` retorna alguma viagem com `baseCurrency !== 'BRL'`; `WalletPage.tsx` redireciona pra `/` sozinho se acessado direto por URL sem nenhuma viagem qualificada (efeito rodando depois de todos os hooks, nunca um early return antes — quebraria as Rules of Hooks).

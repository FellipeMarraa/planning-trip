...
- **`DayRouteMap.tsx`** (`TripItineraryPage.tsx`) mostra só as atividades do dia ativo que têm `coordinates`, ordenadas por horário, com uma `Polyline` reta ligando os pontos — **não é rota de rua/GPS real** (exigiria um serviço pago de directions, fora do escopo desta entrega). Vale tanto pra atividade criada manualmente quanto pra sugestão da IA confirmada (ver seção 8 — a IA nunca marca coordenada sozinha, `api/ai/_lib/geocoding.ts` geocodifica o texto do local que ela sugere antes do usuário confirmar o card).

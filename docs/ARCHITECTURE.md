...
- **A própria moeda da despesa é o sinal — sem campo/checkbox separado**: `useWalletExpenses.ts` filtra `currency !== 'BRL'`. Achado do usuário testando: uma marcação manual (`paidFromWallet`, existiu numa 1ª versão) era redundante — registrar em EUR/GBP/etc. já significa "preciso ter esse dinheiro físico no destino", nunca "posso pagar em real". Consequência: `AddExpenseDialog.tsx` não pergunta mais taxa/spread pra despesa não-BRL (removido o editor de "Cotação de compra"/presets Wise/Cartão) — `amountBRL` sempre usa cotação de mercado, sem escolha manual, porque não existe mais o conceito de "conversão na hora com taxa de cartão" separado da carteira.
...
- **Necessário é a cota do pool, não o valor cheio da despesa nem quem pagou**: achado real — uma despesa de €750 dividida entre 3 pessoas, registrada de uma vez (`participants` com os 3 uids), deve contar só a cota de quem está no pool (€250 se 1 pessoa do pool participa, €500 se 2), nunca os €750 inteiros nem só pra quem é `paidBy`. `useWalletExpenses.ts` busca por `where('participants','array-contains-any', poolUids)` (não `paidBy`) e `summarizeWalletDemand(lots, expenses, poolUids)` calcula `amountOriginal / participants.length` × quantos uids do pool estão em `participants` daquela despesa. Cada pessoa precisa ter em mãos a própria cota, independente de quem historicamente pagou a conta.
...
- **Tela global, não por-URL-de-viagem** (`/wallet`, `WalletPage.tsx`) — busca lotes/despesas-carteira de `[o próprio uid, ...todo parceiro mútuo de qualquer viagem]` numa lista só achatada (`useCurrencyLots`/`useWalletExpenses`, sem filtro de `tripId`) e agrupa/filtra por viagem no componente, usando `useUserTrips()` (já existia).

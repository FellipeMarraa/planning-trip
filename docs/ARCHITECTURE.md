...
- **Tela global na URL (`/wallet`), mas leitura sempre por viagem**: `WalletPage.tsx` lista as viagens não-BRL do usuário (`useUserTrips()`) e renderiza um `TripWalletSection` por viagem — cada instância chama `useCurrencyLots(trip.id)` e `useTrip(trip.id)` (reaproveitado, já buscava `expenses` por `tripId`) **próprios**, nunca uma leitura única achatada cruzando uid de parceiro mútuo. Achado de segurança real: uma query de lista no Firestore é **rejeitada por inteiro** (não filtrada documento a documento) quando o campo do `where` não corresponde ao que a regra de `allow read` avalia — `where('ownerUid','in',[...múltiplos uids])` em `currency_lots` e `where('participants','array-contains-any',[...])` em `expenses` falhavam com `Missing or insufficient permissions`, porque as regras de ambas checam `tripId` (`hasMutualWalletShare(resource.data.tripId,...)` / `isParticipant(resource.data.tripId)`), não esses campos. Corrigido trocando toda query da carteira pra `where('tripId','==', tripId)` — quem é dono do lote/participa do pool na despesa é filtrado client-side depois da leitura (`poolUids.includes(...)`), nunca na query. Ver [SECURITY.md](./SECURITY.md).

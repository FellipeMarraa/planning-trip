...
| `currency_lots` | auto | `services/currencyLots.ts` | `useCurrencyLots` (por `tripId` **e** `ownerUid in poolUids` juntos — nenhum sozinho passa na regra, ver 2.5) |
...
**Não existe campo separado marcando "despesa de carteira"** — o sinal é a própria `currency`: qualquer despesa não-BRL já é, por definição, demanda de carteira de câmbio do pagador (`WalletPage.tsx`/`TripWalletSection` reaproveita `useTrip(tripId)` e filtra `currency !== 'BRL'` client-side, ver 2.5). Achado do usuário testando: uma marcação manual separada (`paidFromWallet`, existiu numa versão anterior desta feature) era redundante — se você registra em EUR/GBP/etc. é porque precisa ter aquele dinheiro físico no destino, nunca "posso pagar em real"; a moeda escolhida já é o sinal, sem precisar de um segundo campo pra confirmar a mesma coisa.
...
Registro de compra de moeda estrangeira, por viagem e por participante (não persiste entre viagens). É aritmética de planejamento, não um caixa: `src/lib/currencyWallet.ts` (`summarizeWalletDemand`) soma `amountPurchased` de todos os lotes de uma moeda vs. a **cota do pool** em cada despesa não-BRL daquela viagem (`amountOriginal / participants.length`, multiplicada por quantos uids do pool estão em `participants` — nunca o valor cheio da despesa nem só a de quem é `paidBy`, ver [ARCHITECTURE.md](./ARCHITECTURE.md) 8.2), e mostra a diferença ("falta comprar X") — nada é consumido/reservado de verdade, então não existe `update` nem lógica de reversão (lote errado é apagado e recriado). `ratePaidBRL` é só informativo.

**Privado por padrão**: `firestore.rules` só libera leitura/escrita pro próprio `ownerUid`, ou pro owner/editor da viagem quando `ownerUid` é um fantasma (`ghost_*`, que não tem login pra gerenciar a própria carteira) — **ou** quando existe compartilhamento mútuo ativo (ver 2.6 `wallet_shares`). A tela `/wallet` (`src/pages/WalletPage.tsx`) lista as viagens não-BRL do usuário (`useUserTrips`) e, pra cada uma, monta um componente `TripWalletSection` próprio que busca **por `tripId`** (`useCurrencyLots(tripId)` + `useTrip(tripId)` reaproveitado pras despesas) — o filtro de "quem é dono/participa do pool" (`poolUids`, o próprio uid + parceiros mútuos daquela viagem) é aplicado client-side depois da leitura, nunca como `where` da query. **Achado importante (2 rodadas de bug pra entender de verdade)**: uma query de lista no Firestore só passa se a regra puder ser provada pro **conjunto potencial de resultados** da query (a forma da query, não o dado real de cada doc) — nunca um filtro silencioso por documento. A regra de `currency_lots` é um OR que depende de **dois** campos, `tripId` e `ownerUid` (`ownerUid == auth.uid` OU `hasMutualWalletShare(tripId, auth.uid, ownerUid)`) — filtrar só por `tripId` deixa `ownerUid` livre (poderia ser de qualquer participante da viagem, que a regra reprova) e nega tudo; filtrar só por `ownerUid in poolUids` deixa `tripId` livre (poderia ser lote de qualquer viagem em que o dono tenha lote) e também nega tudo. A query precisa restringir **os dois campos ao mesmo tempo** (`where('tripId','==',tripId), where('ownerUid','in',poolUids)`) pro Firestore conseguir provar a regra pro conjunto exato pretendido. Isso não é regra geral de "sempre filtrar por tripId" — é regra por regra: `expenses` só depende de `tripId` (`isParticipant(resource.data.tripId)`), então filtrar só por `tripId` (`useTrip`) já basta lá; `currency_lots` depende de `tripId` **e** `ownerUid` juntos, então precisa dos dois. Ver [SECURITY.md](./SECURITY.md). Só acessível (item de menu + rota) por quem participa de ao menos 1 viagem com `baseCurrency !== 'BRL'` (`Layout.tsx`/`WalletPage.tsx`).
